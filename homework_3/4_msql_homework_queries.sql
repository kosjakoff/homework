...
/*
 ...
 Посчитать сколько активных пользователей живет в каждой стране и каждом городе
*/

SELECT `city`.`id`, `city`.`name`, COUNT(*) AS `total_user`
FROM `city` 
    INNER JOIN `city_user` ON (`city`.`id` = `city_user`.`city_id`)
    INNER JOIN `user` ON (`user`.`id` = `city_user`.`user_id`)
...
/*
Посчитать сколько активных пользователей живет в каждой стране и в каждом городе которые авторизовались за последние трое суток.
*/
...
FROM `city` 
    ...
    INNER JOIN `authorization` ON (`authorization`.`user_id` = `city_user`.`user_id`)
    INNER JOIN `user` ON (`user`.`id` = `city_user`.`user_id`)
    WHERE `user`.`is_active` = 1 AND (TO_DAYS(NOW( )) - TO_DAYS(`date`)) <= 3
GROUP BY `city_user`.`city_id`; 
...
/*
Найти ту страну и тот город с которого максимальное количество авторизаций активных пользователей за (все время/за последние три дня/за последний месяц/квартал/год)
*/
...
FROM (
    ...
    FROM `city` 
        ...
        INNER JOIN `user` ON (`user`.`id` = `city_user`.`user_id`)
    WHERE `user`.`is_active` = 1 
	AND (QUARTER(NOW( )) = QUARTER(`date`)) 
    AND (YEAR(NOW( )) = YEAR(`date`))
    GROUP BY `country`.`id`
) `country_user_authorization`;

/*
Вывести список стран по возрастанию количества активных пользователей в стране. (Аналогично сделать для городов)
*/
...
/*
Посчитать сколько проживает активных пользователей по ролям в каждом городе/каждой стране
*/
...
/*
Посчитать сколько раз заходили НЕАКТИВНЫЕ пользователи с ролью админ за последний месяц в разрезе каждой страны и города
*/
...
/*
Найти всех НАКТИВНЫХ пользователей у которых нет ни одной роли
*/
...
FROM `user` 
    LEFT JOIN `role_user` ON (`user`.`id` = `role_user`.`user_id`)
WHERE `user`.`is_active` = 0 
AND `role_user`.`role_id` IS NULL;

/*
Посчитать сколько проживает в каждой стране и в каждом городе НАКТИВНЫХ пользователей по всем существующим ролям
*/
...
FROM `role` 
    ...
    LEFT JOIN `city_user` ON (`city_user`.`user_id` = `role_user`.`user_id`)
    LEFT JOIN `city` ON (`city`.`id` = `city_user`.`city_id`)
    LEFT JOIN `user` ON (`user`.`id` = `city_user`.`user_id`)
WHERE `user`.`is_active` = 0
AND `role_user`.`role_id` IS NOT NULL
GROUP BY `role`.`id`, `city_user`.`city_id`;
